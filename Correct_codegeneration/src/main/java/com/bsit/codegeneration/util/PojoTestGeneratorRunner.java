...
import com.bsit.codegeneration.parser.PojoTestGenerator;
...
public class PojoTestGeneratorRunner {

    private static final Logger logger = LoggerFactory.getLogger(PojoTestGeneratorRunner.class);
    ...
    private static boolean processTable(Connection conn, String tableName) throws SQLException {
        try (ResultSet columns = DatabaseUtils.getTableColumns(conn, DEFAULT_SCHEMA, tableName)) {
        ...
        try (ResultSet columns = DatabaseUtils.getTableColumns(conn, DEFAULT_SCHEMA, tableName)) {
            List<Relationship> forwardRels = DatabaseUtils.discoverRelationships(conn, DEFAULT_SCHEMA, tableName, false);
            List<Relationship> reverseRels = DatabaseUtils.discoverRelationships(conn, DEFAULT_SCHEMA, tableName, true);

            System.out.println("  forward relationships: " + forwardRels.size());
            System.out.println("  reverse relationships: " + reverseRels.size());

            PojoTestGenerator.generateDtoTest(
                    tableName,
                    columns,
                    dbConfig,
                    targetConfig,
