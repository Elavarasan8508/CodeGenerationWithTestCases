...
public class JdbcStaffDao {
    ...
    private static final String SELECT_BY_ID_SQL = """
        ...
        """.formatted("staff_id, first_name, last_name, address_id, email, store_id, active, username, password, last_update, picture", TABLE, COL_STAFF_ID);
    ...
    private static final String SELECT_BY_ADDRESS_ID_SQL = """
        SELECT %s FROM %s WHERE %s = ?
        """.formatted("staff_id, first_name, last_name, address_id, email, store_id, active, username, password, last_update, picture", TABLE, COL_ADDRESS_ID);

    private static final String SELECT_BY_STORE_ID_SQL = """
        SELECT %s FROM %s WHERE %s = ?
        """.formatted("staff_id, first_name, last_name, address_id, email, store_id, active, username, password, last_update, picture", TABLE, COL_STORE_ID);
    ...
    private void setStaffParams(PreparedStatement ps, Staff staff) throws SQLException {
        ps.setObject(1, staff.getFirstName(), Types.VARCHAR);
        ps.setObject(2, staff.getLastName(), Types.VARCHAR);
        if (staff.getAddress() != null) {
            ps.setObject(3, staff.getAddress().getAddressID(), Types.INTEGER);
        } else {
            ps.setNull(3, Types.INTEGER);
        }
        ps.setObject(4, staff.getEmail(), Types.VARCHAR);
        if (staff.getStore() != null) {
            ps.setObject(5, staff.getStore().getStoreID(), Types.INTEGER);
        } else {
        ...
        ps.setObject(7, staff.getUsername(), Types.VARCHAR);
        ps.setObject(8, staff.getPassword(), Types.VARCHAR);
        java.time.LocalDateTime val9 = staff.getLastUpdate();
        if (val9 != null) {
            ps.setObject(9, java.sql.Timestamp.valueOf(val9), Types.TIMESTAMP);
        } else {
            ps.setNull(9, Types.TIMESTAMP);
        }
        ps.setObject(10, staff.getPicture(), Types.VARCHAR);
    }
