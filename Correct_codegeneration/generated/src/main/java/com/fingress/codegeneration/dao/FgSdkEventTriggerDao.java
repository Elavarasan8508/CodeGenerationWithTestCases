...
import com.bsit.codegeneration.mapper.FgSdkEventTriggerMapper;
import java.util.*;

@RegisterRowMapper(FgSdkEventTriggerMapper.class)
public interface FgSdkEventTriggerDao {

    @SqlQuery("SELECT * FROM FG_SDK_EVENT_TRIGGER")
    public List<FgSdkEventTrigger> findAll();

    @SqlQuery("SELECT * FROM FG_SDK_EVENT_TRIGGER WHERE id = :id")
    public Optional<FgSdkEventTrigger> findById(@Bind("id") String id);
