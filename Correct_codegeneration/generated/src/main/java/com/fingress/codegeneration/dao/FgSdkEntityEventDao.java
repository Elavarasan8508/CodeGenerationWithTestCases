...
import com.bsit.codegeneration.mapper.FgSdkEntityEventMapper;
import java.util.*;

@RegisterRowMapper(FgSdkEntityEventMapper.class)
public interface FgSdkEntityEventDao {

    @SqlQuery("SELECT * FROM FG_SDK_ENTITY_EVENT")
    public List<FgSdkEntityEvent> findAll();

    @SqlQuery("SELECT * FROM FG_SDK_ENTITY_EVENT WHERE id = :id")
    public Optional<FgSdkEntityEvent> findById(@Bind("id") String id);
