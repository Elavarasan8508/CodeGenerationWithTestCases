...
import com.bsit.codegeneration.mapper.FgTokenCloneMapper;
import java.util.*;

@RegisterRowMapper(FgTokenCloneMapper.class)
public interface FgTokenCloneDao {

    @SqlQuery("SELECT * FROM FG_TOKEN_CLONE")
    public List<FgTokenClone> findAll();

    @SqlQuery("SELECT * FROM FG_TOKEN_CLONE WHERE id = :id")
    public Optional<FgTokenClone> findById(@Bind("id") String id);
